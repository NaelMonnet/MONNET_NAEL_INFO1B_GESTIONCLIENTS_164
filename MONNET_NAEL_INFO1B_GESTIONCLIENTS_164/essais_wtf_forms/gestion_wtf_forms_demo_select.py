...
from MONNET_NAEL_INFO1B_GESTIONCLIENTS_164.erreurs.msg_erreurs import *
from MONNET_NAEL_INFO1B_GESTIONCLIENTS_164.essais_wtf_forms.wtf_forms_demo_select import DemoFormSelectWTF
...
def demo_select_wtf():
    ...
    # Objet formulaire pour montrer une liste déroulante basé su    r la table "t_personne"
    form_demo = DemoFormSelectWTF()
    try:
        print("form_demo.submit_btn_ok_dplist_personne.data  ", form_demo.submit_btn_ok_dplist_personne.data)
        if request.method == "POST" and form_demo.submit_btn_ok_dplist_personne.data:

            if form_demo.submit_btn_ok_dplist_personne.data:
        ...
        if request.method == "GET":
            ...
            for i in data_genres:
                genre_val_list_dropdown.append(i['nom_pers'])

            # Aussi possible d'avoir un id numérique et un texte en correspondance
            # genre_val_list_dropdown = [(i["id_personne"], i["nom_pers"]) for i in data_genres]
