...
from MONNET_NAEL_INFO1B_GESTIONCLIENTS_164.erreurs.exceptions import *
    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_mail, nom_mail,
                                                            GROUP_CONCAT(nom_pers, ' ' ,prenom_pers) as GenresFilms FROM t_pers_avoir_mail
                                                            RIGHT JOIN t_mail ON t_mail.id_mail = t_pers_avoir_mail.fk_mail
                                                            LEFT JOIN t_personne ON t_personne.id_personne = t_pers_avoir_mail.fk_personne
                                                            GROUP BY id_mail"""
                if id_film_sel == 0:
                    ...
                    valeur_id_film_selected_dictionnaire = {"value_id_mail_selected": id_film_sel}
                    # En MySql l'instruction HAVING fonctionne comme un WHERE... mais doit être associée à un GROUP BY
                    # L'opérateur += permet de concaténer une nouvelle valeur à la valeur de gauche préalablement définie.
                    strsql_genres_films_afficher_data += """ HAVING id_mail= %(value_id_mail_selected)s"""
...
"""
    nom: edit_personne_film_selected
    ...
    1) Tous les genres contenus dans la "t_personne".
...
@app.route("/edit_personne_film_selected", methods=['GET', 'POST'])
def edit_personne_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
            ...
            print("dans edit_personne_film_selected ---> data_genres_all", data_genres_all)
            ...
            # href="{{ url_for('edit_personne_film_selected', id_film_genres_edit_html=row.id_film) }}"
            ...
            lst_data_genres_films_non_attribues = [item['prenom_pers'] for item in data_genres_films_non_attribues]
            print("lst_all_genres gf_edit_personne_film_selected ", lst_data_genres_films_non_attribues,
                  type(lst_data_genres_films_non_attribues))

        except Exception as Exception_edit_personne_film_selected:
            raise ExceptionEditGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                 f"{edit_personne_film_selected.__name__} ; "
                                                 f"{Exception_edit_personne_film_selected}")
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            new_lst_int_personne_film_old = list(map(int, new_lst_str_genres_films))
            print("new_lst_personne_film ", new_lst_int_personne_film_old, "type new_lst_personne_film ",
                  type(new_lst_int_personne_film_old))

            # Pour apprécier la facilité de la vie en Python... "les ensembles en Python"
            # https://fr.wikibooks.org/wiki/Programmation_Python/Ensembles
            # OM 2021.05.02 Une liste de "id_personne" qui doivent être effacés de la table intermédiaire "t_personne_film".
            lst_diff_genres_delete_b = list(set(old_lst_data_genres_films_attribues) -
                                            set(new_lst_int_personne_film_old))
            print("lst_diff_genres_delete_b ", lst_diff_genres_delete_b)

            # Une liste de "id_personne" qui doivent être ajoutés à la "t_personne_film"
            lst_diff_genres_insert_a = list(
                set(new_lst_int_personne_film_old) - set(old_lst_data_genres_films_attribues))
            print("lst_diff_genres_insert_a ", lst_diff_genres_insert_a)

            # SQL pour insérer une nouvelle association entre
            # "fk_film"/"id_film" et "fk_genre"/"id_personne" dans la "t_personne_film"
            strsql_insert_personne_film = """INSERT INTO t_pers_avoir_mail (id_pers_avoir_mail, fk_personne, fk_mail)
                                                    VALUES (NULL, %(value_fk_genre)s, %(value_fk_film)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "id_film" et "id_personne" dans la "t_personne_film"
            strsql_delete_genre_film = """DELETE FROM t_pers_avoir_mail WHERE fk_personne = %(value_fk_personne)s AND fk_mail = %(value_fk_film)s"""

            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_personne_film".
                ...
                for id_genre_ins in lst_diff_genres_insert_a:
                    ...
                    mconn_bd.execute(strsql_insert_personne_film, valeurs_film_sel_genre_sel_dictionnaire)

                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_personne_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
    ...
    # Après cette mise à jour de la table intermédiaire "t_personne_film",
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_mail, nom_mail, GROUP_CONCAT(id_personne) as GenresFilms FROM t_pers_avoir_mail
                                        INNER JOIN t_mail ON t_mail.id_mail = t_pers_avoir_mail.fk_mail
                                        INNER JOIN t_personne ON t_personne.id_personne = t_pers_avoir_mail.fk_personne
                                        WHERE id_mail = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_personne, prenom_pers FROM t_personne WHERE id_personne not in(SELECT id_personne as idGenresFilms FROM t_pers_avoir_mail
                                                    INNER JOIN t_mail ON t_mail.id_mail = t_pers_avoir_mail.fk_mail
                                                    INNER JOIN t_personne ON t_personne.id_personne = t_pers_avoir_mail.fk_mail
                                                    WHERE id_mail = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_mail, id_personne, prenom_pers FROM t_pers_avoir_mail
                                            INNER JOIN t_mail ON t_mail.id_mail = t_pers_avoir_mail.fk_mail
                                            INNER JOIN t_personne ON t_personne.id_personne = t_pers_avoir_mail.fk_personne
                                            WHERE id_mail = %(value_id_film_selected)s"""
