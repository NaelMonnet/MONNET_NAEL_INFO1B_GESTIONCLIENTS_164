...
from MONNET_NAEL_INFO1B_GESTIONCLIENTS_164.films.gestion_films_wtf_forms import FormWTFUpdateFilm, FormWTFAddFilm, \
    FormWTFDeleteFilm
    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_mail (id_mail, nom_mail) VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update_film.submit.data:
            ...
            str_sql_update_nom_film = """UPDATE t_mail SET nom_mail = %(value_nom_film)s
                                                            WHERE id_mail = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_mail WHERE id_mail = %(value_id_film)s"
            ...
            form_update_film.nom_film_update_wtf.data = data_film["nom_mail"]
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_pers_avoir_mail WHERE fk_mail = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_mail WHERE id_mail = %(value_id_film)s"""
        ...
        if request.method == "GET":
            valeur_select_dictionnaire = {"value_id_mail": id_film_delete}
